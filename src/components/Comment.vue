        ...
        <div class="container">
            <div class="comment comment-card">
                ...
                <div class="comment-controls">
                    <a v-if="currentUser" class="delete" @click="deleteComment(index)"><img src="../assets/images/icon-delete.svg" alt="" class="control-icon">Delete</a>
            ...
            <div class="commet-modal comment-card" v-if="reply">
                ...
                <button class="btn btn-primary" @click="submitReply" :data-for="index">reply</button>
            </div>
        </div>
        <div class="replies comments-wrapper" v-if="comment.replies.length > 0">
            <Reply 
            v-for="(comment ,index) in comment.replies" 
            :comment="comment" 
            :data-index="index" 
            :comment-pos="commentpos"
...
export default {
    ...
    methods: {
        ...mapMutations(['ADD_REPLY','DELETE_COMMENT','TOGGLE_DELETE']),
        editComment() {
            this.edit =true;
        },
        submitComment() {
        ...
        submitReply(e) {
            ...
            const commentObj = {
                comment :newReply,
                index:e.target.getAttribute('data-for')
            };
            this.ADD_REPLY(commentObj);
            this.reply= false;
        },
        reciveReplyObj(replyObj) {
